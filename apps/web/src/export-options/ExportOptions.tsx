...
function downloadAction(
  ...
  const filename = `${paletteTitle?.toLowerCase().replace(/\W/g, '-') || 'primer'}.css`
  const type = 'text'
...
function downloadPaletteAsImage(
  ...
  dlLink.href = dataUrl
  dlLink.download = `${
    colorNames.fetchedData?.paletteTitle?.toLowerCase().replace(/\W/g, '-') || 'primer'
  }.png`
...
function downloadCodeTheme(theme: CodeThemeOutput, paletteTitle: string, cb: (msg: string, type: 'success' | 'error') => void) {
  ...
  const filename = `${paletteTitle?.toLowerCase().replace(/\W/g, '-') || 'primer'}.json`
...
export function ExportOptions({ fetchedData, isLoading, error, colorFormat, setColor }: ExportOptionsProps) {
  ...
  function handleFileDownload() {
    if (mode === 'code') {
      if (codeTheme) downloadCodeTheme(codeTheme, colorNames.fetchedData?.paletteTitle || 'primer', showMessage)
      return
    }
  ...
  return (
    <div className="export-options-container">
      <Button handler={handleImageDownload} active={false} tooltip="Download palette as a PNG image">
        <ImageIcon size={22} color="url(#image-gradient)" weight="fill">
          <LinearGradientSVG id="image-gradient" />
        </ImageIcon>
      </Button>
      <Button handler={handleFileDownload} active={false} tooltip="Download palette as a CSS file">
        <FileArrowDownIcon size={22} color="url(#file-arrow-down-gradient)" weight="fill">
          <LinearGradientSVG id="file-arrow-down-gradient" />
        </FileArrowDownIcon>
      </Button>
      <Button handler={handleCopyToClipboard} active={false} tooltip="Copy CSS variables to clipboard">
      ...
      <Button
        ...
        active={false}
        tooltip="Copy a shareable link to this palette"
      >
      ...
      <Button
        ...
        active={false}
        tooltip="Generate a random palette"
      >
