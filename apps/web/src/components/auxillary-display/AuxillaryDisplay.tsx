...
import type { ColorFormat, ColorSpace, CodeThemeOutput } from 'primer'
import { BaseColorData } from 'primer'
import { ColorHistory } from '../color-history/ColorHistory'
import { ColorContext } from '../ColorContext'
import { MockupsContainer } from '../mockups/MockupsContainer'
...
function CodeThemeDetails({ codeTheme, showText }: { codeTheme: CodeThemeOutput; showText: boolean }) {
  const { showMessage } = useContext(MessageContext)
  const [scope, animate] = useAnimate()
...
export function AuxillaryDisplay({
  ...
  if (mode === 'mockups') {
    return <MockupsContainer palette={palette} />
  }
  
  return <PaletteDetails palette={palette} colorNames={colorNames} colorSpace={colorSpace} mode={mode} showText={showPaletteColors} />
}
