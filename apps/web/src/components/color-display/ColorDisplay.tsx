...
import { ColorFormat, ColorSpace, PaletteKinds, PaletteStyle } from 'primer'
...
export function ColorDisplay({
  ...
  return (
    <div className="current-color-display flex col align-start" onClick={handleClick}>
      ...
      <div className={`color-formats ${colorSpace.format}`}>
      ...
      <div className="color-status-chips">
        <span className="status-chip">
          <SwatchesIcon weight="fill" size={12} color={context.palette[1]?.color?.clone()?.to('lch')?.set({ l: 80 })?.display() || 'var(--dimmed)'} />
          {paletteTypeFull}
        </span>
        <span className="status-chip">
          <CirclesFourIcon weight="fill" size={12} color={context.palette[2]?.color?.clone()?.to('lch')?.set({ l: 80 })?.display() || 'var(--dimmed)'} />
          {paletteStyle} Variant
        </span>
        <span className={`status-chip${effectsEnabled ? ' active' : ''}`}>
          <FadersHorizontalIcon
            weight="fill"
            size={12}
            color={effectsEnabled && context.palette[3] ? context.palette[3].color.clone().to('lch').set({ l: 80 }).display() : 'var(--dimmed)'}
          />
          {effectsEnabled ? 'Effects On' : 'No Effects'}
        </span>
      </div>
    </div>
  )
